...
--filmlerde oynayan aktörler getirilecek,
--tüm aktörler değil. o zaman left veya right join kullanacağız

select film_name,category,actor_name from filmler left join aktorler on films_id = film_id;

--films_id ile film_id aynı olsaydı;
--filmler.film_id = aktorler.film_id
--şeklinde yazılırdı
--------------------------------------------------------------------

--Soru-2
--tüm actor_name leri ve oynadigi film name leri listeleyiniz
--esas tablomuz : aktörler

select actor_name,film_name from filmler right join aktorler on films_id = film_id;

--veya

select actor_name,film_name from aktorler LEFT join filmler on films_id = film_id;
...
--------------------------------------------------------------------
--soru 2 : customer name product ve price field'larının hepsini getiriniz
...
SELECT e.name employee_name, m.name manager_name FROM Employees e INNER JOIN Employees m on 
...
--ILIKE olsa case sensitive olmaz. A veya a ile başlayan (Ignore Case)

--------------------------------------------------------------------
--soru 2: isimleri 'i'harfi ile biten ogrencilerin bilgilerini getirin
...
SELECT * FROM ogrenciler
WHERE isim ILIKE '%z%' OR isim ILIKE '%k%';
